...
**Goal:** Let a customer place a rental order on the website, track it through an eight-state lifecycle at a no-login reference link, and give the owner a single queue containing exactly the orders that need them.
...
- Payment is verified by hand — no GCash webhook, no auto-confirmation (spec: Decisions #3).
...
- Out of scope: customer accounts/passwords, GCash webhook, automated credential release, account/game swaps, migrating any other collection, and notifications beyond the on-page status and owner queue (spec: Out of scope for v1).
...
- Modify: `server.js:10` (add the require), `server.js:364-402` (export the existing Mongo getter to the module)
...
- Produces, all from `lib/orders.js`:
  ...
  - `nextRef()` → `Promise<string>` — allocates and returns the next `PH-NNNN` code.
  ...
  - `transition(ref, toState, patch)` → `Promise<order|null>` — validates the move against `canTransition`, applies `patch`, appends to `state_history`.
  - `canTransition(from, to)` → `boolean` — pure, no I/O, exported for the test script.
  - `expireStaleQrs()` → `Promise<number>` — moves every `qr_pending` order whose `qr_expires_at` has passed back to `awaiting_qr`, returns how many moved.
  - `advanceEndedRentals()` → `Promise<number>` — moves every `active` order whose `end_date` is in the past to `awaiting_return`, returns how many moved.
  - `markRefunded(ref)` → `Promise<boolean>` — sets `deposit_refunded: true` on a closed order. Not a lifecycle move, so it bypasses `transition()`.
...
async function getByRef(ref) {
  const col = await _col('orders');
  if (!col) return null;
  return col.findOne({ ref: String(ref).toUpperCase() });
}
...
module.exports = {
  ...
  init, canTransition, nextRef, create, getByRef, listByStates, transition,
  expireStaleQrs, advanceEndedRentals, markRefunded
};
```
...
Run: `node scripts/test-orders.js`
Expected: PASS — eleven `ok -` lines followed by `11 assertions passed`.
...
### Task 2: Order creation from the game page
...
**Interfaces:**
- Consumes: `orders.create(fields)` from Task 1; the existing `resolveGamePrices(game)` (`server.js:523`), `getPriceCategory(id)` (`server.js:516`), `getSiteSettings()` (`server.js:563`), `getPromoDiscountPct(promo, days)`, and `gameSlug(title)` (`server.js:929`).
- Produces: order documents carrying `ref`, `game_id`, `game_title`, `account_type`, `days`, `price_tier_name`, `price_snapshot`, `amount_due`, `deposit_due`, `fb_name` — consumed by Tasks 3-6. Redirects to `/order/:ref`, the route Task 3 builds.
...
### Task 3: Customer status page and payment proof
...
**Interfaces:**
- Consumes: `orders.getByRef(ref)`, `orders.transition(ref, toState, patch)`, `orders.expireStaleQrs()` from Task 1; order documents created by Task 2; the existing `processUploadedImage(file, maxDim)` helper (`server.js:214`) and `getSiteSettings()`.
- Produces: the `/order/:ref` page that Tasks 4-6 extend with further step UI. Establishes `uploadOrderFile` (a `multer` instance writing into `uploadsDir`), reused by Task 4's QR upload and Task 5's return upload.

- [ ] **Step 1: Add the multer instance and the status route**

In `server.js`, immediately after the `POST /order/create` route added in Task 2, insert:
...
<%
  const STEP_COPY = {
    awaiting_payment:  { title: 'Send your payment',       sub: 'Pay via GCash, then send us proof below.' },
  ...
  const mmText = 'Paying for order ' + order.ref + ' — ' + order.game_title + ', ' + typeLabel + ', ' + durLabel + ', PHP ' + totalDue;
%>
...
<div class="ord-page">
  ...
  <% if (msg === 'payment_submitted') { %><div class="ord-flash">Proof received — we'll confirm shortly.</div><% } %>
  ...
  <div class="ord-step">
    <div class="ord-step-label">Send your proof</div>
    <a class="ord-btn-primary" target="_blank" rel="noopener"
       href="http://m.me/PlaystationHub00?text=<%= encodeURIComponent(mmText) %>">
    ...
    <form method="POST" action="/order/<%= order.ref %>/payment-proof" enctype="multipart/form-data" class="ord-upload">
      ...
      <button type="submit" class="ord-btn-secondary">Upload receipt here</button>
    </form>
...
.ord-btn-primary { display: block; width: 100%; text-align: center; background: var(--ps-blue); color: #fff; padding: 0.9rem; border-radius: 10px; font-weight: 800; font-size: 0.95rem; text-decoration: none; }
.ord-btn-secondary { display: block; width: 100%; background: #1a1a1a; color: #ddd; border: 1.5px solid #2a2a2a; border-radius: 10px; padding: 0.8rem; font-weight: 800; font-size: 0.9rem; cursor: pointer; font-family: inherit; margin-top: 0.5rem; }
...
.ord-upload { margin-top: 0.9rem; }
.ord-upload input[type="file"] { width: 100%; font-size: 0.8rem; color: #888; }
...
git commit -m "$(cat <<'EOF'
Add customer order status page with dual-channel payment proof

/order/:ref renders the current step with no login required. Payment
proof can arrive via a prefilled Messenger link carrying the reference
code, or as a direct upload — either way the order lands in the same
verifying_payment state so nothing bypasses the owner's queue.
EOF
)"
```
...
### Task 4: QR upload with countdown and expiry
...
**Interfaces:**
- Consumes: `orders.transition()`, `orders.QR_WINDOW_MS`, `orders.getByRef()` from Task 1; `uploadOrderFile` and `processUploadedImage()` from Task 3; the `ownerOnline` local already passed to the view by Task 3's `GET /order/:ref`.
- Produces: orders reaching `qr_pending` with `qr_expires_at` set, which Task 5's owner queue sorts by.

- [ ] **Step 1: Add the QR upload route**

In `server.js`, immediately after the `POST /order/:ref/payment-proof` route from Task 3, insert:
...
In `views/order-status.ejs`, immediately after the closing `<% } %>` of the payment step block added in Task 3, insert:
...
In `views/order-status.ejs`, find the flash line added in Task 3:
...
### Task 5: Owner queue, verify actions, and the online toggle

**Files:**
- Modify: `server.js` (add the queue data to the `/admin` render, plus `POST /admin/orders/:ref/advance`, `POST /admin/orders/:ref/reject`, `POST /admin/online`)
...
**Interfaces:**
- Consumes: `orders.listByStates()`, `orders.OWNER_STATES`, `orders.transition()`, `orders.expireStaleQrs()` from Task 1; `uploadOrderFile` and `processUploadedImage()` from Task 3; the existing `requireAuth` middleware (`server.js:304`) and `getSiteSettings()`.
- Produces: `site_settings.owner_online` (boolean), read by Task 4's status page via the `ownerOnline` local. Orders reaching `closed` with `deposit_due > 0` and `deposit_refunded: false`, which Task 6 lists.

- [ ] **Step 1: Add the return-proof route and the owner action routes**

In `server.js`, immediately after the `POST /order/:ref/qr` route from Task 4, insert:
...
<% } else { %>
  ...
  <div class="oq-row oq-<%= o.state %>">
    <div class="oq-main">
      ...
      <div class="oq-fb">FB: <strong><%= o.fb_name %></strong></div>
...
In `views/order-status.ejs`, immediately after the `qr_pending` block from Task 4, insert:
...
advance. The rental clock starts when the owner signs the customer in
rather than at order time, so an overnight wait isn't billed. The
online toggle drives the live banner on customers' status pages.
EOF
)"
```
...
### Task 6: Deposit refund tracking
...
**Interfaces:**
- Consumes: `refundsOwed` (already computed and passed to the admin view by Task 5, Step 2); `orders.markRefunded(ref)` from Task 1.
- Produces: nothing consumed downstream — this is the final surface.

- [ ] **Step 1: Add the mark-refunded route**

In `server.js`, immediately after the `POST /admin/online` route from Task 5, insert:
...
- [ ] **Step 1: Confirm the Mongo connection is configured**

Run: `node scripts/test-orders.js`
Expected: PASS — `9 assertions passed`.
...
- The page shows the payment step with both a Messenger button and an upload control.

- [ ] **Step 5: Verify the Messenger prefill carries the reference**

Read the `href` of the "Send proof on Messenger" link and confirm the decoded `text` parameter contains the order's reference code, game title, account type, duration, and total.

- [ ] **Step 6: Walk the order through every state**

Click "I already sent it on Messenger" to reach `verifying_payment`. Then in `/admin` (password from project context), open the Orders tab and confirm the order appears with the customer's Facebook name and a "Payment confirmed" button. Advance it, then back on the status page upload any image as the QR, confirm the countdown appears and counts down, then advance again from admin and confirm the status page shows the rental as active with an end date `days` ahead of today.

- [ ] **Step 7: Verify the QR expiry loop**

Place a second test order, advance it to `awaiting_qr`, upload a QR, then use the Browser tool's `javascript_tool` to check that `orders` sweeping works by reloading `/admin?tab=orders` after the 10-minute window — or, to avoid a 10-minute wait, confirm the sweep logic instead by checking that the queue row shows a live countdown that decrements, and that the status page reloads itself when the countdown reaches zero.

- [ ] **Step 8: Verify the deposit refund list**
...
- [ ] **Step 9: Confirm nothing existing broke**

Load `/`, `/browse`, and a game page. Confirm the original "Message Us on Facebook" CTA still works unchanged, the booking panel behaves as before, and the admin's existing tabs (Games, Customers, PS Plus) all still render.

- [ ] **Step 10: Report results to the user**

Summarize what was verified in Steps 4-9, with screenshots of the customer status page and the owner queue, and flag anything that didn't match expectations before considering this plan complete.
