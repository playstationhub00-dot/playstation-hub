...
function buildRequestQueue(customers, reviews, now) {
  ...
  const rows = (customers || [])
    ...
    .map(c => {
      const reviewed = hasReviewedBy(reviews, c.customer_name);
      const askedAt = c.review_asked_at || null;
      return {
        id: c.id,
        name: c.customer_name || '',
        // How they would appear on the site, so the ask can promise it exactly.
        displayName: maskName(c.customer_name),
        game: c.game_title || '',
        accountType: c.account_type || '',
        startDate: c.start_date || '',
        status: reviewed ? 'reviewed' : (askedAt ? 'asked' : 'todo'),
        askedAt,
        daysSinceStart: daysSince(c.start_date, at),
