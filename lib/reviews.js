...
// Reviews used to be one to five stars. They are now a thumb: a customer either
// recommends us or does not. Five points of nuance were never worth the extra
// tap on a phone, and nobody was choosing 2 or 3 anyway.
//
// Old reviews stay exactly as they are on disk and are read through this. The
// star rating is still WRITTEN alongside the thumb, so the admin dashboard's
// average and the moderation list keep working without a migration.
const STAR_FOR = Object.freeze({ up: 5, down: 1 });
const RECOMMEND_FROM_STARS = 4;
const NEUTRAL_STARS = 3;

// Returns 'up', 'down', or null for "they never gave a clear answer".
//
// null exists only for old star data. Exactly 3 out of 5 is the one rating that
// does not translate: calling it a recommend overstates, calling it a complaint
// understates, and both put a wrong number on the homepage. It votes in
// neither direction instead. Nothing submitted through the current form can be
// null — a thumb is always one or the other.
function sentimentOf(review) {
  const r = review || {};
  if (r.sentiment === 'up' || r.sentiment === 'down') return r.sentiment;
  const stars = Number(r.rating);
  // No rating at all means a review the owner typed in from a Facebook comment
  // they chose to publish. Reading that as negative would invent criticism
  // nobody wrote.
  if (!stars) return 'up';
  if (stars === NEUTRAL_STARS) return null;
  return stars >= RECOMMEND_FROM_STARS ? 'up' : 'down';
}

// Customer-supplied thumb and comment, made safe to store. The thumb falls back
// to 'up' rather than rejecting: a missing radio should not lose someone's
// written comment, and inventing a complaint is the worse failure. The comment
// is optional — one tap is a complete answer, and demanding a sentence is why
// most people abandon a review form.
function normalize(input) {
  const raw = input || {};
  let sentiment = raw.sentiment === 'down' ? 'down' : raw.sentiment === 'up' ? 'up' : null;
  // A cached page could still post the old star radio.
  if (!sentiment && raw.rating != null && raw.rating !== '') {
    const stars = parseInt(raw.rating, 10);
    if (!isNaN(stars)) sentiment = stars >= RECOMMEND_FROM_STARS ? 'up' : 'down';
  }
  if (!sentiment) sentiment = 'up';
  let text = String(raw.text == null ? '' : raw.text).trim().replace(/\s+/g, ' ');
  if (text.length > MAX_TEXT) text = text.slice(0, MAX_TEXT).trim();
  return { sentiment, rating: STAR_FOR[sentiment], text };
}
...
// How many distinct people have actually rented — the figure shown beside the
// star rating on every public page. Counts PEOPLE, not rental records: one
// customer with fourteen rentals is one renter, or the word "renters" would be
// overstating the business. Deliberately reuses REQUESTABLE_STATUSES and
// normalizeName, so this number can never disagree with the ask queue about
// who counts as a renter or about which two rows are the same person.
function countRenters(customers) {
  const seen = new Set();
  (customers || []).forEach(c => {
    if (!c || !REQUESTABLE_STATUSES.includes(c.status)) return;
...
// Turns an exact renter count into the figure shown publicly: rounded DOWN to
// the nearest 50, with a "+". Down, never up — "300+" has to remain true, and
// rounding up would advertise customers who don't exist. It also stops the
// headline reading "301+" the day after it reads "300+": the number only moves
// when a real milestone is passed.
//
// Below the first milestone the exact count is shown with no "+", because
// there is nothing honest to round to — 12 renters is not "50+", and "0+"
// is meaningless.
const RENTER_MILESTONE_STEP = 50;
function renterMilestone(count) {
  const n = Math.floor(Number(count) || 0);
  if (!(n > 0)) return { n: 0, plus: false };
  if (n < RENTER_MILESTONE_STEP) return { n, plus: false };
  return { n: Math.floor(n / RENTER_MILESTONE_STEP) * RENTER_MILESTONE_STEP, plus: true };
}
...
// How many of the people who told us anything would recommend us — the figure
// shown on every public page in place of a star average.
//
// Counted over EVERY review submitted, not just the visible ones. Negative
// reviews are routed privately and never published, so a ratio counted over the
// visible pool would read 100% forever, which would be a lie by construction.
function recommendStats(reviews) {
  // Anyone whose answer does not translate is left out of BOTH halves of the
  // fraction, so an old 3-star import cannot drag the figure down or prop it up.
  const list = (reviews || []).filter(r => r && sentimentOf(r) !== null);
  if (!list.length) return { up: 0, total: 0, pct: 0 };
  const up = list.filter(r => sentimentOf(r) === 'up').length;
  return { up, total: list.length, pct: Math.round((up / list.length) * 100) };
}
...
module.exports = {
  ...
  sentimentOf, recommendStats,
  hasReviewedBy, buildRequestQueue, queueSummary, countRenters, renterMilestone
};
