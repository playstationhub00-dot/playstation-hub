...
function render(templateText, customer, templates, opts) {
  ...
  const reviewLink = (c.order_ref && c.order_key)
    ...
    : '';
  // Two wordings for the two moments it is asked. Both collapse to '' without
  // a link, so the parent template's trailing-blank trim removes the line
  // entirely rather than leaving a dangling sentence.
  const reviewLine = reviewLink ? fill(templates.review_line, { review_link: reviewLink }) : '';
  const reviewLineSetup = reviewLink ? fill(templates.review_line_setup, { review_link: reviewLink }) : '';
  ...
  const values = {
    ...
    // The extension itself, passed in by the caller. `days` and `price` above
    // are the running totals after it, which alone read like the original
    // booking — these are what actually changed today. Rendered as '0' rather
    // than blank when a free extension is given, so the line still reads.
    ext_days: o.extDays != null ? String(o.extDays) : '',
    ext_price: o.extPrice != null ? String(o.extPrice) : '',
    ...
    review_line_setup: reviewLineSetup,
...
const DEFAULT_TEMPLATES = Object.freeze({
  // Extending a rental. {days} and {price} are the running totals AFTER the
  // extension, which on their own read like the original booking — so this
  // leads with {ext_days} and {ext_price}, what was actually added and paid
  // today, and the new return date.
  extension:
`✅ Extension confirmed, {name}!

🎮 Game: {game}
➕ Added: {ext_days} more days
💰 Paid today: ₱{ext_price}
📅 New return date: {end_date}

Nothing to do on your side — stay signed in, your time just moved.

{deposit_line}`,
  // A sale, not a rental. Deliberately carries no {days} and no {end_date}:
  // the rental confirmation was being sent to buyers, telling someone who owns
  // the game outright how many days they had it and when to give it back.
  purchase:
`✅ All yours, {name}!
...
👤 Account: {type}
💰 Paid: ₱{price}
♾️ Yours to keep — no return date.
...
{deposit_line}`,
  ...
  // Sent once the account is back and the deposit has gone out. The only
  // message in the set that asks nothing of the customer, which is exactly why
  // the review ask sits here rather than buried under return instructions.
  return_complete:
`✅ All done, {name} — thanks for renting with us!

🎮 {game} ({type})
💰 Your ₱{deposit} deposit is on its way back.
...
`Just delete the account from your console:
  ...
  // Both ask about the SERVICE — speed, set-up, replies — because that is what
  // customers actually write here ("legit and fast", "zero issues with the
  // activation"). They already knew the game before they rented it, which is
  // why asking at sign-in time is fair: by then they have had the whole of what
  // they are being asked to rate.
  review_line_setup: '⭐ How was it so far? If the ordering and set-up were quick, a 10-second review really helps new customers trust us:\n{review_link}',
  review_line: '⭐ How was renting with us? Fast, easy, no hassle? A quick review takes 10 seconds and helps the next customer:\n{review_link}',
