...
const ALLOWED = {
  ...
  // Reservation orders (paid Coming Soon reservations) settle into 'reserved'
  ...
  //
  // The one edge out exists to undo a mis-click on the owner's "priority paid"
  // button, which promotes a free waitlist entry when the ₱100 arrived over
  // Messenger. Without it that button is a one-way door: 'cancelled' does not
  // accept 'reserved', so the only remedy would be deleting the order and
  // losing the customer's ref, position and history.
  //
  // The edge is deliberately NOT a general "un-reserve". A Coming Soon
  // reservation also rests in 'reserved', and that customer paid real money for
  // a game, not a ₱100 queue fee — sending them back to a free waitlist entry
  // would erase a genuine payment. The state machine cannot express that
  // difference, so POST /admin/orders/:ref/undo-priority guards on
  // upgraded_from_waitlist and is the only caller.
  reserved:          ['waitlisted'],
...
// Defaults to now. Callers already write `manilaDate()` meaning "today" —
// Quick Add does it for an order with no start date given — and without the
// default that threw a TypeError deep inside the route, surfacing as a bare
// "Could not save that" with the real cause only in the server log.
...
async function create(fields) {
  ...
  // An order normally starts at awaiting_payment and walks the state machine
  // from there. The admin's Quick Add is the exception: it records a rental
  // that already happened — money in, account already on the customer's
  // console — so it is BORN at its true state. Walking it there through
  // transitions would need the machine to allow jumps it must not allow, and
  // would write a state_history of sign-in steps that never took place.
  const startState = (fields && fields.state) || 'awaiting_payment';
  const order = Object.assign({
    ...
    end_date: '',
    created_at: now,
    state_history: [{ state: startState, at: now }]
  }, fields);
...
// Webhook replay protection. Lives in Mongo beside orders rather than in the
// lowdb blob, because that blob is rewritten whole on every save — two webhooks
// arriving together would race and one record would be lost, which is exactly
// the case this exists to prevent.
async function wasEventProcessed(eventId) {
  if (!eventId) return false;
  const col = await _col('payment_events');
  if (!col) return false;
  return !!(await col.findOne({ _id: String(eventId) }));
}

// Recorded before the order is touched. If the process dies midway the event
// reads as processed and the payment is left for reconciliation, which is the
// safe direction: a payment visible in admin beats one applied twice.
async function recordEvent(eventId, meta) {
  if (!eventId) return false;
  const col = await _col('payment_events');
  if (!col) return false;
  try {
    await col.insertOne(Object.assign({ _id: String(eventId), at: new Date().toISOString() }, meta || {}));
    return true;
  } catch (e) {
    // Duplicate key means another delivery of the same event won the race.
    if (e && e.code === 11000) return false;
    throw e;
  }
}

// The gateway's checkout session id, kept as a second way to tie a webhook back
// to its order if the reference and metadata are ever both missing. A plain
// field write with no state-machine meaning, so it does not go through
// transition().
// Webhook health, so a wrong PAYMONGO_WEBHOOK_SECRET is visible instead of
// silent. Going live means registering a NEW webhook in PayMongo's live mode
// with its own secret; carry the test one over by mistake and every real
// payment fails signature verification, leaving customers charged and orders
// stuck while nothing but a server log says so.
//
// Deliberately stores counters and timestamps only — never any part of an
// unverified request body, since a failed signature means the content is
// exactly what cannot be trusted.
async function noteWebhook(ok) {
  const col = await _col('payment_health');
  if (!col) return false;
  const now = new Date().toISOString();
  const inc = ok ? { ok_count: 1 } : { fail_count: 1 };
  const set = ok ? { last_ok_at: now } : { last_fail_at: now };
  await col.updateOne({ _id: 'webhook' }, { $inc: inc, $set: set }, { upsert: true });
  return true;
}
...
async function webhookHealth() {
  const col = await _col('payment_health');
  if (!col) return null;
  return col.findOne({ _id: 'webhook' });
}

async function setCheckoutSession(ref, checkoutId) {
  const col = await _col('orders');
  if (!col) return false;
  const clean = parseOrderRef(ref);
  if (!clean || !checkoutId) return false;
  await col.updateOne({ ref: clean }, { $set: { checkout_session_id: String(checkoutId) } });
  return true;
}
...
async function expireStaleQrs() {
  ...
  for (const o of stale) {
    // Stamped so the customer's page can say what happened. Without this the
    // order simply reappears at "send your code" with no explanation, which
    // reads as though nothing was ever received — and the usual next move is
    // to message and ask, rather than to send a fresh code.
    const r = await transition(o.ref, 'awaiting_qr', {
      qr_image: null,
      qr_expires_at: null,
      qr_expired_at: new Date().toISOString(),
      qr_expired_count: (Number(o.qr_expired_count) || 0) + 1
    });
    if (r) moved++;
  }
  return moved;
...
// Turns an order that was recorded as a rental back into the purchase it
// always was. A direct write, like deleteOrder: the state machine has no edge
// for "this was never a rental", and walking one would invent history.
//
// Clearing end_date is the load-bearing part — advanceEndedRentals() sweeps any
// active order past its end date into 'awaiting_return', which is how a
// customer who owns the game outright gets asked to give it back. If that
// sweep already ran, this puts the order back to active.
async function repairPurchase(ref) {
  const col = await _col('orders');
  if (!col) return false;
  const clean = parseOrderRef(ref);
  if (!clean) return false;
  const current = await getByRef(clean);
  if (!current) return false;
  const set = { days: null, end_date: '', is_buy: true };
  const update = { $set: set };
  if (current.state === 'awaiting_return' || current.state === 'verifying_return') {
    const at = new Date().toISOString();
    set.state = 'active';
    update.$push = { state_history: { state: 'active', at } };
  }
  const r = await col.updateOne({ ref: clean }, update);
  return r.matchedCount > 0;
}

// Moves an order's rental window. A direct write, like deleteOrder and
// repairPurchase: extending is not a state change, so transition() has no edge
// for it.
//
// end_date is the load-bearing field. advanceEndedRentals() sweeps any active
// order past its end date into 'awaiting_return', so an extension that updated
// only the customer row left the order still expiring on the original date —
// the customer's own link kept counting down to the old date and asked them to
// send return proof for days they had just paid for.
//
// Also clears a stale return: if that sweep already ran, the order is sitting in
// awaiting_return for a rental that now has time left, so it goes back to active.
// The fields an owner edit is allowed to push onto an order, and nothing else.
// A whitelist rather than a spread: the caller builds this object out of form
// input, and a stray field reaching the order document is how a record grows
// keys nobody meant to write.
const WINDOW_FIELDS = Object.freeze(['end_date', 'days', 'amount_due']);
// Everything on the customer form that the customer's own order page renders.
// See views/order-status.ejs: game, account type, duration, rent and both
// dates all read off the ORDER, never off the customer row.
const CUSTOMER_FIELDS = Object.freeze(
  WINDOW_FIELDS.concat(['start_date', 'account_type', 'game_id', 'game_title'])
);

async function _applyOrderPatch(ref, patch, allowed) {
  const col = await _col('orders');
  if (!col) return false;
  const clean = parseOrderRef(ref);
  if (!clean) return false;
  ...
  const set = {};
  allowed.forEach(k => {
    if (patch && patch[k] !== undefined) set[k] = patch[k];
  });
  if (!Object.keys(set).length) return false;
  const update = { $set: set };
  // Shortening a rental can leave the end date in the past; advanceEndedRentals
  // sweeps that into awaiting_return on its own. Lengthening one that the sweep
  // already took has to be undone here, or the customer keeps being asked to
  // return a game they have paid to keep.
  const stillRunning = set.end_date && set.end_date >= manilaDate(new Date());
  if (stillRunning && (current.state === 'awaiting_return' || current.state === 'verifying_return')) {
    const at = new Date().toISOString();
    set.state = 'active';
    update.$push = { state_history: { state: 'active', at } };
  }
...
async function setRentalWindow(ref, patch) {
  return _applyOrderPatch(ref, patch, WINDOW_FIELDS);
}

// Push an owner's customer-row edit onto the order behind it. The customer row
// and the order hold the same rental twice, and only the order is what the
// customer's link renders — so editing a rental from monthly to weekly used to
// change the admin table while the customer's own page kept showing the old
// duration, price and return date. Extend already did this; the edit form,
// which is the other way all the same fields change, did not.
async function syncFromCustomer(ref, patch) {
  return _applyOrderPatch(ref, patch, CUSTOMER_FIELDS);
}
...
module.exports = {
  ...
  wasEventProcessed, recordEvent, setCheckoutSession, noteWebhook, webhookHealth,
