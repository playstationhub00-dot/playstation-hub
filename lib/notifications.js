...
const RANK = Object.freeze({
  ...
  unhappy_customer: 1,   // the most perishable thing here: it gets harder to fix hourly
  qr_pending: 2,         // a 10-minute clock is running
  verifying_payment: 3,  // someone is watching a screen, waiting to be let in
  expiry_overdue: 4,
  verifying_return: 10,
  expiry_today: 11,
  no_slot: 12,
...
function build(input) {
  ...
  // ── An extension the order never heard about ──
  // Extending used to mean hand-editing the customer row, which left the order
  // still expiring on the original date. advanceEndedRentals() then swept it
  // into 'awaiting_return' early, so the customer's own link asked them to
  // return a game they had just paid to keep. Fixed at the source; this row is
  // for the rentals extended before that.
  (src.staleEndDates || []).forEach(o => {
    if (!o) return;
    push({
      id: 'stale_order_end:' + o.ref, kind: 'stale_order_end', urgency: 'warn', icon: '📅',
      title: 'Extension never reached the order',
      sub: detail([o.ref, nameOf(o), o.game_title,
                   o.customer_end_date ? 'should run to ' + o.customer_end_date : 'order expires early']),
      tab: 'customers', ref: o.ref, tie: 0
    });
  });

  // ── A purchase filed as if it were a rental ──
  // Quick Add's mode field was hardcoded to 'rent', so picking "Bought" left
  // the order with a duration and an end date. advanceEndedRentals() sweeps any
  // active order past its end date into 'awaiting_return' — so on that date the
  // site asks someone who owns the game outright to give it back, and it turns
  // up here as a return to verify. Fixed at the source; this row is for the
  // records made before that.
  (src.boughtWithDuration || []).forEach(o => {
    if (!o) return;
    push({
      id: 'bought_with_duration:' + o.ref, kind: 'bought_with_duration', urgency: 'warn', icon: '🛒',
      title: 'Purchase filed as a rental',
      sub: detail([o.ref, nameOf(o), o.game_title,
                   o.end_date ? 'will ask for a return on ' + o.end_date : 'carries a rental duration']),
      tab: 'customers', ref: o.ref, tie: 0
    });
  });

  // ── An order whose customer record was deleted out from under it ──
  // Before the delete routes were made symmetric, removing a cancelled
  // customer left the order behind. It keeps counting in the Orders ledger's
  // monthly total, and if it is still in a live state a sweep will drag it
  // into the return queue for a customer who no longer exists. The fix is to
  // delete the order too — this row is the reminder to do that.
  (src.orphanedOrders || []).forEach(o => {
    if (!o) return;
    const money = (Number(o.amount_due) || 0) + (Number(o.deposit_due) || 0);
    push({
      id: 'orphaned_order:' + o.ref, kind: 'orphaned_order', urgency: 'warn', icon: '🧹',
      title: 'Order with no customer record',
      sub: detail([o.ref, nameOf(o), o.game_title,
                   money ? peso(money) + ' still in your monthly total' : 'delete it from Orders']),
      tab: 'orders', ref: o.ref, tie: 0
    });
  });

  // ── Money quietly left on the table ──
  // A rental charged for one duration and written down as another: they paid a
  // week's rate and hold the account for a month. Grouped into a single row on
  // purpose. The first time this runs it may find a batch of them, and twenty
  // separate rows would bury the queue items that are on a clock — the row is
  // a pointer to the full list on the Customers tab, where each one can be
  // read in context before anything is changed.
  const mismatches = (src.rentMismatches || []).filter(Boolean);
  if (mismatches.length) {
    const short = mismatches.reduce((sum, m) => sum + (Number(m.shortfall) || 0), 0);
    const worst = mismatches[0];
    push({
      id: 'rent_mismatch', kind: 'rent_mismatch', urgency: 'warn', icon: '⚖️',
      title: mismatches.length === 1
        ? 'A rental is priced short'
        : mismatches.length + ' rentals are priced short',
      sub: detail([
        peso(short) + ' under across ' + (mismatches.length === 1 ? 'it' : 'them'),
        worst ? 'worst: ' + (nameOf(worst) || worst.game_title || 'unnamed') : '',
  ...
  // ── Someone told us it went badly ──
  // A thumbs down never reaches the site; it comes here instead, and stays
  // until it is marked handled. That is the whole deal the review form makes
  // with the customer, so it has to be a queue item and not a chat message
  // that scrolls away.
  (src.negativeReviews || []).forEach(r => {
    if (!r) return;
    push({
      id: 'unhappy_customer:' + r.id, kind: 'unhappy_customer', urgency: 'critical', icon: '💔',
      title: 'Unhappy customer',
      sub: detail([String(r.name || '').trim(), r.game_rented, r.order_ref,
                   r.text ? '“' + r.text + '”' : 'no comment left']),
      tab: 'content', tie: ms(r.created_at)
    });
  });
