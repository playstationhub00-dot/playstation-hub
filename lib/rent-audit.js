...
// A ceiling on the loop below, not a real-world limit on a rental. maxDays is
// a number read straight out of a customer record — Number(customer.days) —
// with no upper bound of its own before it reaches here. A single malformed
// or fat-fingered row (an extra zero, a corrupted migration, Infinity from
// bad arithmetic elsewhere) turned this into an unbounded loop that ran on
// EVERY admin page load for every renting customer, on Node's single thread,
// hanging the entire app rather than just failing one request — this brought
// production down. Ten years is generously past anything a real rental could
// ever be, so nothing legitimate is cut off by capping here.
const MAX_LOOP_DAYS = 3650;
...
function daysCoveredBy(price, tier, maxDays) {
  const cap = Number.isFinite(maxDays) ? Math.min(maxDays, MAX_LOOP_DAYS) : 0;
  let best = 0;
  for (let d = 1; d <= cap; d++) {
