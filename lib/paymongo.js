  ...
  return {
    data: {
      attributes: {
        ...
        // Ask for everything this business could ever accept; PayMongo shows
        // the customer only the ones actually active on the account. That means
        // a method approved later starts appearing on its own, with no code
        // change — which matters here because GCash needs a business-type
        // upgrade and Maya is still under review, while QRPh is already live.
        // QRPh is the national QR standard: a customer pays it by scanning with
        // GCash, Maya or their bank app, so it reaches the same wallets even
        // while those wallets are not enabled as methods in their own right.
        payment_method_types: o.paymentMethods || ['qrph', 'gcash', 'paymaya', 'card'],
