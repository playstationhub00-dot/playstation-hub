...
// The public request list is the one place a customer's name is shown to other
// customers. Initials keep the social proof ("real people asked for this") while
// giving away nothing identifying. The admin list still shows full names — the
// owner has to message these people.
//
// Two words at most, so a four-part Filipino name does not render as J.M.D.C.
function initials(fbName) {
  const parts = String(fbName || '').trim().split(/\s+/)
    .map(w => w.trim()[0])
    // A name of only punctuation or emoji has no letter to abbreviate; dropping
    // those here is what stops the result becoming a bare '.'
    .filter(ch => ch && /[\p{L}\p{N}]/u.test(ch))
    .slice(0, 2);
  if (!parts.length) return 'Someone';
  return parts.map(ch => ch.toUpperCase() + '.').join('');
}
...
async function renameVoter(slug, at, newName) {
  // Validated before touching the database — a blank name is rejectable without
  // a round trip, and the caller gets 'empty' rather than a misleading 'no_db'.
...
module.exports = {
  ...
  init, slugify, firstName, initials, ensureIndexes,
